...
地域住民や観光客が「今ここで何が起きているか」をひと目で把握できる地域密着型 Web アプリを作成する。

### 背景

- 福島県内のイベント情報が複数サイトに分散しており、まとめて確認できる場所がない
- IT 系イベント（勉強会・ハッカソン）は Connpass に掲載されているが、福島で絞り込む手段が限られている
...
| # | 機能名 | 詳細 | 受け入れ条件 | 実装状況 |
|---|--------|------|-------------|---------|
| F-07 | 絞り込み検索 | 地域・カテゴリ・日付（開始〜終了）でイベントを絞り込める | 条件を指定すると該当イベントのみ表示される | 🚧 未実装 |
| F-08 | カレンダー表示 | FullCalendar でイベントを月・週単位で表示する | イベントがカレンダー上に表示され、クリックで詳細に遷移できる | 🚧 未実装 |
...
| メインカラー | グリーン系（自然・地域をイメージ）またはブルー系（清潔感） |
...
| 項目 | 方針 |
|------|------|
| イベントカード | 画像・タイトル・日時・カテゴリを一目で把握できるデザイン |
| 検索フォーム | トップに配置、すぐに絞り込みできる UI |
| 全体の雰囲気 | 地域の人が気軽に使えるシンプルで読みやすい UI |
